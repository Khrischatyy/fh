...
build:
	...
	@$(DOCKER_COMPOSE) -f dev.yml build

dev:
	@echo "$(GREEN)Starting all services...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml up

dev-detach:
	@echo "$(GREEN)Starting all services in background...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml up -d
	@echo ""
...
dev-build:
	...
	@$(DOCKER_COMPOSE) -f dev.yml up --build

start: dev

stop:
	@echo "$(YELLOW)Stopping all services...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml down
...
status:
	...
	@$(DOCKER_COMPOSE) -f dev.yml ps

logs:
	@$(DOCKER_COMPOSE) -f dev.yml logs -f $(container)

logs-all:
	@$(DOCKER_COMPOSE) -f dev.yml logs -f

logs-api:
	@$(DOCKER_COMPOSE) -f dev.yml logs -f api

logs-celery:
	@$(DOCKER_COMPOSE) -f dev.yml logs -f celery_worker

logs-frontend:
	@$(DOCKER_COMPOSE) -f dev.yml logs -f frontend

logs-caddy:
	@$(DOCKER_COMPOSE) -f dev.yml logs -f caddy
...
migrate:
	...
	@$(DOCKER_COMPOSE) -f dev.yml exec api alembic upgrade head
...
migrate-create:
	...
	@$(DOCKER_COMPOSE) -f dev.yml exec api alembic revision --autogenerate -m "$(message)"

migrate-down:
	@echo "$(YELLOW)Rolling back last migration...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml exec api alembic downgrade -1
	@echo "$(GREEN)Rollback complete$(NC)"

migrate-history:
	@$(DOCKER_COMPOSE) -f dev.yml exec api alembic history

test:
	@echo "$(GREEN)Running FastAPI tests...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml exec api pytest tests/ -v --cov=src --cov-report=term-missing

test-verbose:
	@$(DOCKER_COMPOSE) -f dev.yml exec api pytest tests/ -vv
...
shell:
	...
	@$(DOCKER_COMPOSE) -f dev.yml exec api python

shell-bash:
	@$(DOCKER_COMPOSE) -f dev.yml exec api /bin/sh

routes:
	@echo "$(GREEN)Listing all FastAPI routes...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml exec api python scripts/list_routes.py
...
db-shell:
	...
	@$(DOCKER_COMPOSE) -f dev.yml exec db psql -U postgres -d book_studio

db-reset:
	@echo "$(YELLOW)⚠️  WARNING: This will delete all data!$(NC)"
	@echo -n "Are you sure? [y/N] " && read ans && [ $${ans:-N} = y ]
	@echo "$(YELLOW)Resetting database...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml exec api alembic downgrade base
	@$(DOCKER_COMPOSE) -f dev.yml exec api alembic upgrade head
	@echo "$(GREEN)Database reset complete$(NC)"

seed:
	@echo "$(GREEN)🌱 Running database seeders...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml exec api python -m src.db.seed
...
npm-install:
	...
	@$(DOCKER_COMPOSE) -f dev.yml run --rm frontend sh -c "npm i"

npm-install-package:
	@$(DOCKER_COMPOSE) -f dev.yml run --rm frontend sh -c "npm i ${p}"

update-frontend:
	@echo "$(GREEN)Restarting frontend container...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml stop frontend
	@$(DOCKER_COMPOSE) -f dev.yml up -d frontend
...
celery-status:
	...
	@$(DOCKER_COMPOSE) -f dev.yml exec celery_worker celery -A src.tasks.celery_app inspect active

celery-stats:
	@echo "$(GREEN)Celery worker statistics...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml exec celery_worker celery -A src.tasks.celery_app inspect stats

celery-purge:
	@echo "$(YELLOW)Purging all Celery tasks...$(NC)"
	@$(DOCKER_COMPOSE) -f dev.yml exec celery_worker celery -A src.tasks.celery_app purge -f
...
prod-update:
	...
	@$(DOCKER_COMPOSE) -f prod.yml down
	@echo ""
	@echo "$(YELLOW)Step 3/5: Rebuilding containers...$(NC)"
	@$(DOCKER_COMPOSE) -f prod.yml build --no-cache
	@echo ""
	...
	@sleep 10
	@$(DOCKER_COMPOSE) -f prod.yml exec -T api alembic upgrade head || echo "$(YELLOW)Migration skipped or failed$(NC)"
	@echo ""
	@echo "$(GREEN)=========================================$(NC)"
...
prod-rebuild-frontend:
	...
	@$(DOCKER_COMPOSE) -f prod.yml stop frontend
	@$(DOCKER_COMPOSE) -f prod.yml build --no-cache frontend
	@$(DOCKER_COMPOSE) -f prod.yml up -d frontend
	@echo "$(GREEN)✅ Frontend rebuilt and restarted!$(NC)"
	@echo "$(BLUE)Tailing logs (Ctrl+C to exit)...$(NC)"
	@$(DOCKER_COMPOSE) -f prod.yml logs -f frontend

prod-update-frontend:
	@echo "$(YELLOW)Updating frontend (quick rebuild)...$(NC)"
	@$(DOCKER_COMPOSE) -f prod.yml stop frontend
	@$(DOCKER_COMPOSE) -f prod.yml build frontend
	@$(DOCKER_COMPOSE) -f prod.yml up -d frontend
...
stop-prod:
	@$(DOCKER_COMPOSE) -f prod.yml stop
...
status-prod:
	...
	@$(DOCKER_COMPOSE) -f prod.yml ps

logs-prod:
	@$(DOCKER_COMPOSE) -f prod.yml logs -f $(container)

logs-prod-all:
	@$(DOCKER_COMPOSE) -f prod.yml logs -f

logs-prod-api:
	@$(DOCKER_COMPOSE) -f prod.yml logs -f api

logs-prod-frontend:
	@$(DOCKER_COMPOSE) -f prod.yml logs -f frontend

logs-prod-caddy:
	@$(DOCKER_COMPOSE) -f prod.yml logs -f caddy

logs-prod-celery:
	@$(DOCKER_COMPOSE) -f prod.yml logs -f celery_worker

migrate-prod:
	@echo "$(GREEN)Running production migrations...$(NC)"
	@$(DOCKER_COMPOSE) -f prod.yml exec -T api alembic upgrade head
	@echo "$(GREEN)✅ Migrations applied successfully!$(NC)"

seed-prod:
	@echo "$(GREEN)🌱 Running production database seeders...$(NC)"
	@$(DOCKER_COMPOSE) -f prod.yml exec -T api python -m src.db.seed
	@echo "$(GREEN)✅ Seeders completed!$(NC)"
...
clean:
	...
	@$(DOCKER_COMPOSE) -f dev.yml down -v
	@$(DOCKER_COMPOSE) -f prod.yml down -v
	@echo "$(GREEN)Cleanup complete$(NC)"

clean-prod:
	@$(DOCKER_COMPOSE) -f prod.yml down --remove-orphans
